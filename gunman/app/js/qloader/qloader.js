...
QLoader.plugin('requireProfile', function(profileName, callback) {
    for (var i=0; i < QLoader.profiles.length; i++) {
        if (QLoader.profiles[i].name === profileName) {
            
            QLoader.init(QLoader.profiles[i].sources, callback);
            QLoader.load();
        }
    }
});