 ...
 *
 *
 * @provides fbp.content
 * @requires fbp.prelude fbp.array
 */
...
Quantum.provide('Content', {
  ...
  append: function(content, root) {
    ...
    if (!root) {
      if (!Quantum.Content._root) {
        Quantum.Content._root = root = Quantum.$('fbp-root');
        if (!root) {
          Quantum.log('The "fbp-root" div has not been created.');
          return;
        } else {
          root.className += ' fbp_reset';
        }
      } else {
        root = Quantum.Content._root;
      }
    }
  ...
  appendHidden: function(content) {
    if (!Quantum.Content._hiddenRoot) {
      var
      ...
      Quantum.Content._hiddenRoot = Quantum.Content.append(hiddenRoot);
    }

    return Quantum.Content.append(content, Quantum.Content._hiddenRoot);
  },
  ...
  insertIframe: function(opts) {
    ...
    //
    opts.id = opts.id || Quantum.guid();
    opts.name = opts.name || Quantum.guid();
    ...
    var
      ...
      srcSet = false,
      onloadDone = false;
    Quantum.Content._callbacks[guid] = function() {
...
//#JSCOVERAGE_IF
    if (document.attachEvent) {
      var html = (
        '<iframe' +
          ...
          ' onload="Quantum.Content._callbacks.' + guid + '()"' +
        '></iframe>'
      );
      ...
      node.id = opts.id;
      node.name = opts.name;
      node.onload = Quantum.Content._callbacks[guid];
  ...
  postTarget: function(opts) {
    ...
    form.method = 'POST';
    Quantum.Content.appendHidden(form);

    Quantum.Array.forEach(opts.params, function(val, key) {
  ...
  /**
   * Dinamycally create an image and preload it.
   */
  
  preloadImage: function(_url) {
